...
# --------------------------------------------------
    ...
    st.title("🧠 Ask Nuvora — AI Career Assistant")
    st.write("👋 Hi! I'm Nuvora, your AI career assistant. Ask about resumes, ATS, or interviews.")
    ...
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            ...
            st.markdown(f"<div class='bot-msg'>🤖 Nuvora: {msg['content']}</div>", unsafe_allow_html=True)
        ...
        if ai_available:
            try:
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
                    messages=[
                        {"role": "system", "content": "You are Nuvora, an AI career assistant helping with resumes, ATS, and interviews."},
                        *st.session_state.chat_history
                    ]
                )
